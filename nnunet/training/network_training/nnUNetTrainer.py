...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.loss = DC_and_CE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {},
                                   ignore_label=cfg.ignore_class_label)  # YK
        print(f'treating {cfg.ignore_class_label} as label to be ignored')
        ...
        self.oversample_foreground_percent = cfg.data_loader.oversample_foreground_percent  # ori: 0.33
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        from nnunet.evaluation.evaluation_utils import process_pred_mask
        current_mode = self.network.training
        self.network.eval()
        ...
        self.print_to_log_file('output folder:', output_folder)
        ...
        for ii, k in enumerate(self.dataset_val.keys()):
            ...
            if overwrite or (not isfile(join(output_folder, fname + ".nii.gz"))) or \
                ...
                print(f"{ii}/{len(self.dataset_val)}", k, data.shape)
                data[-1][data[-1] == -1] = 0
                ...
                output_path_raw = join(output_folder, fname + ".nii.gz")
                output_path_postproc = join(output_folder+'_postproc', fname + ".nii.gz")
                maybe_mkdir_p(output_folder+'_postproc')

                results.append(export_pool.starmap_async(save_segmentation_nifti_from_softmax,
                                                         ((softmax_pred, output_path_postproc,
                                                           properties, interpolation_order, self.regions_class_order,
                                                           # None, None,
                                                           process_pred_mask, [properties['itk_spacing']],
                                                           softmax_fname, output_path_raw, force_separate_z,
                                                           interpolation_order_z, cfg.validation.verbose),
                                                          )
                                                         )
                               )
    ...
    def my_online_evaluation(self, output, target):
        ...
        for pred, gt in zip(output, target):
            ...
            num_pred = min(num_pred, 100)  # avoid computing for too long time
    ...
    def finish_online_evaluation(self):
        ...
        if cfg.my_online_val and cfg.cur_epoch % 2 == 0:
            self.my_finish_online_evaluation()
    ...
    def my_finish_online_evaluation(self):
        ...
        sens, fp_per_vol, precisions, sens_per_class = \
            FROC_mask(ovlps, all_scores, ovlp_th, ignore_masks, list(range(1, num_lesion_cls+1)), gt_classes)
