...
from nnunet.inference.post_process import process_pred_mask
...
def preprocess_save_to_queue(preprocess_fn, q, list_of_lists, output_files, segs_from_prev_stage, classes,
    ...
    for i, l in enumerate(list_of_lists):
        try:
            ...
            if cfg.validation.verbose: print("preprocessing", output_file)
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    if not cfg.validation.verbose: sys.stdout = open(os.devnull, 'w')  # YK: suppress original outputs
    ...
    all_output_files = []
    for ii, preprocessed in enumerate(preprocessing):
        ...
        sys.__stdout__.write(f"predicting {ii}/{len(list_of_lists)}: " + os.path.basename(output_filename)+'\n')
        sys.__stdout__.flush()
        trainer.load_checkpoint_ram(params[0], False)
        if cfg.data_loader.get('smooth_normal_im', False):
            d = scipy.signal.convolve(d[0], np.ones((1,2,2))/4, 'same')[None]
        ...
        output_folder, fname = os.path.dirname(output_filename), os.path.basename(output_filename)
        output_path_raw = join(output_folder, fname)
        output_path_postproc = join(output_folder + '_postproc', fname)
        maybe_mkdir_p(output_folder + '_postproc')

        results.append(pool.starmap_async(save_segmentation_nifti_from_softmax,
                                          ((softmax,
                                            # trainer.num_classes,
                                            output_path_postproc, dct, interpolation_order, region_class_order,
                                            # None, None,
                                            process_pred_mask, [dct['itk_spacing']],
                                            npz_file, output_path_raw, force_separate_z, interpolation_order_z,
                                            cfg.validation.verbose),)
                                          ))
    ...
    pool.close()
    pool.join()
    if not cfg.validation.verbose: sys.stdout = sys.__stdout__
