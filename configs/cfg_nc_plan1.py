exp_name = 'MICCAI paper, task 286, NC cls+det+seg+consist loss'
results_dir = "./nnUNet_trained_models/PLAN/" + exp_name
#TODO Note: According to PLAN paper, first train with this config for 500 epochs, then train with cfg_nc_plan2 for 500
...
#   also be inferred. If validation.folders already contains outputs, the inferred files will be skipped, so remember
#   to delete the old validation.folders if you want to re-infer the validation set.
...
model = dict(
    ...
    add_anchor_query=True,  # see PLAN paper, add anchor queries to det branch generated from seg branch
    ...
    #     train_start_ep=50,  # It seems good to start learning the patient branch after learning the det/seg branch
    #     # for several epochs
...
# inference data
data_fd = '/data/vdd/user/codes/nnUNet/nnUNet_raw_data_base/nnUNet_raw_data/Task286_Task284_MICCAI23/'
...
validation = dict(
    ...
    folders=[
        ...
        (data_fd + 'imagesTs_normal200', output_fd,),
        (data_fd + 'imagesTs_normal_hard100', output_fd, 'eval_sens_spec', data_fd + 'labelsTs'),
    ],
