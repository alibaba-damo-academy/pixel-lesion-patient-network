...
# YK, modify DataLoader3D by adding instance masks
...
class DataLoader3D_Inst(DataLoader3D):
    def __init__(self, is_train, data, patch_size, final_patch_size, batch_size, configs, has_prev_stage=False,
    ...
    def get_do_oversample(self, batch_idx):
        if self.cfg.fg_prob_continuous:  # YK
            return np.random.rand() < self.oversample_foreground_percent  # YK
        else:  # nnUNet origin strategy
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            data_file = self._data[i]['data_file'][:-4]
            filename = data_file + '.npy'
            if isfile(filename):
                try:
                    case_all_data = np.load(filename, self.memmap_mode)
                except:
                    case_all_data = np.load(filename, 'r+')  # rare error, don't know why
            else:
            ...
            if not force_fg:
                ...
                if self.cfg.fg_force_lesion:
                    if np.any(foreground_classes <= self.cfg.num_lesion_classes_in_annot):
                        foreground_classes = foreground_classes[foreground_classes <= self.cfg.num_lesion_classes_in_annot]
                    elif cfg.model.liver_label in foreground_classes:
                        foreground_classes = [cfg.model.liver_label]
        ...
        seg = self.process_mask_classes(seg)
        if self.cfg.preprocess_mask:
            seg, classes = self.preprocess_mask(seg, case_properties)
            return {'data': data, 'seg': seg, 'label': classes, 'properties': case_properties, 'keys': selected_keys}
        else:
            seg = self.remove_small_CC(seg, case_properties)
            return {'data': data, 'seg': seg,  'properties': case_properties, 'keys': selected_keys}

    def preprocess_mask(self, masks, case_properties):
        """Put this here since data loading is multi threaded, maybe faster than put in the loss
        Set very small lesion instances to ignore.
        Split lesions to instances and record the class of each instance.
        Class 0 is bg"""
        classes = []
        for b in range(len(masks)):
            mask = masks[b, 0].astype(int)
            # ignore_mask = mask == self.cfg.ignore_class_label
            ori_max_cls = mask.max()
            lesion_mask = ((mask > 0) & (mask <= self.cfg.num_lesion_classes))  # label > num_lesion_classes are organ classes
            instance_mask = measure.label(mask * lesion_mask, connectivity=2)
            spacing = case_properties[b]['spacing_after_resampling']
            for i in range(1, instance_mask.max() + 1):
                if (instance_mask == i).sum() < self.cfg.min_train_lesion_size:
                    mask[instance_mask == i] = self.cfg.ignore_class_label
                if (instance_mask == i).sum()*np.prod(spacing) < self.cfg.max_neg_lesion_size_mm3:
                    mask[instance_mask == i] = cfg.model.liver_label
                    instance_mask[instance_mask == i] = 0

            # return instance mask anyway, re-process in loss if we do semantic seg
            masks[b, 0][lesion_mask] = instance_mask[lesion_mask] + ori_max_cls
            # masks[b, 0][ignore_mask] = self.cfg.ignore_class_label
            index_to_label = [(i, mask[masks[b, 0] == i][0])
                            for i in range(instance_mask.max() + ori_max_cls + 1)
                            if np.any(masks[b, 0] == i)]
            index_to_label = np.vstack(index_to_label) if len(index_to_label) > 0 else np.empty((0, 2))
            classes.append(index_to_label)
        return masks, classes

    def process_mask_classes(self, seg):
        """Remove or merge some classes"""
        if 'merge_classes' not in self.cfg or len(self.cfg.merge_classes) == 0:
            return seg
        cls_list = self.cfg.merge_classes
        seg_new = seg * ((seg < 0) | (seg >= len(cls_list)))
        for i, clss in enumerate(cls_list):
            for cls in clss:
                seg_new[seg == cls] = i
        return seg_new

    def remove_small_CC(self, masks, case_properties):
        """Remove small connected components because they may be wrong annotations"""
        for b in range(len(masks)):
            mask = masks[b, 0].astype(int)
            lesion_mask = ((mask > 0) & (mask <= self.cfg.num_lesion_classes))  # label > num_lesion_classes are organ classes
            instance_mask = measure.label(mask * lesion_mask, connectivity=2)
            ...
            for i in range(1, instance_mask.max() + 1):
                if (instance_mask == i).sum() < self.cfg.min_train_lesion_size:
                    masks[b, 0, instance_mask == i] = self.cfg.ignore_class_label
                if (instance_mask == i).sum()*np.prod(spacing) < self.cfg.max_neg_lesion_size_mm3:
                    mask[b, 0, instance_mask == i] = cfg.model.liver_label
        return masks
