...
class NetworkTrainer(object):
    ...
    def do_split(self):
        ...
        splits_file = join(self.dataset_directory, cfg.split_file)
        ...
        self.print_to_log_file('Using split file', splits_file)
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            self.print_to_log_file("\tepoch: ", self.epoch)
            epoch_start_time = time()
            train_losses_epoch = []
            train_loss_dicts_epoch = []  # YK
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch) as tbar:
                    for b in tbar:
                        ...
                        l, loss_dict = self.run_iteration(self.tr_gen, True)
                        tbar.set_postfix(loss=round(float(l), 2),
                                         **{k: round(float(v), 2) for k, v in loss_dict.items()})
                        train_losses_epoch.append(l)
                        train_loss_dicts_epoch.append(loss_dict)

            else:
                for _ in range(self.num_batches_per_epoch):
                    l, loss_dict = self.run_iteration(self.tr_gen, True)
                    # print(self.class_loss.normal_count)
                    train_losses_epoch.append(l)
                    train_loss_dicts_epoch.append(loss_dict)

            self.all_tr_losses.append(np.mean(train_losses_epoch))
            loss_keys = set([k for d in train_loss_dicts_epoch for k in d])
            train_loss_dict = {k: np.nanmean([d[k] for d in train_loss_dicts_epoch if k in d]) for k in loss_keys}
            self.print_to_log_file("train loss : %.4f: %s" % (self.all_tr_losses[-1],
                                   ', '.join(['%s= %.4f' % (k, v) for k, v in train_loss_dict.items()])))
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l, loss_dict = self.run_iteration(self.val_gen, False, True)
                    val_losses.append(l)
                    val_loss_dicts.append(loss_dict)
                self.all_val_losses.append(np.mean(val_losses))
                val_loss_dict = {k: np.nanmean([d[k] for d in val_loss_dicts]) for k in val_loss_dicts[0]}
                self.print_to_log_file("validation loss: %.4f: %s" % (self.all_val_losses[-1],
                                       ', '.join(['%s= %.4f' % (k, v) for k, v in val_loss_dict.items()])))
                ...
                if self.also_val_in_tr_mode:
                    ...
                    val_losses, val_loss_dicts = [], []
                    for b in range(self.num_val_batches_per_epoch):
                        l, loss_dict = self.run_iteration(self.val_gen, False)
                        val_losses.append(l)
                        val_loss_dicts.append(loss_dict)
    ...
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            if self.best_val_eval_criterion_MA is None:
                self.best_val_eval_criterion_MA = -1  # self.val_eval_criterion_MA
